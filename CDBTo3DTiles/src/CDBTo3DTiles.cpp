        ...
        cdb.forEachGeoCell([&](CDBGeoCell geoCell) {
            ...
            cdb.forEachElevationTile(geoCell, [&](CDBElevation elevation) {
                ...
                if (level >= 0) {
                    ...
                    int subtreeRootLevel = (level / subtreeLevels)
                                           * subtreeLevels; // the level of the subtree root
                    ...
                    std::string bufferKey = std::to_string(subtreeRootLevel) + "_"
                                            + std::to_string(subtreeRootX) + "_"
                                            + std::to_string(subtreeRootY);
                    if (subtreeBuffers.find(bufferKey) == subtreeBuffers.end()) // the buffer isn't in the map
                    {
                        subtreeBuffers.insert(
                            std::pair<std::string, std::vector<uint8_t>>(bufferKey,
                                                                         std::vector<uint8_t>(bufferByteLength
                                                                                              * 2)));
                    ...
                    m_impl->addElevationAvailability(elevation,
                                                     cdb,
                                                     nodeAvailabilityBuffer,
                                                     childSubtreeAvailabilityBuffer,
                                                     &subtreeAvailableNodeCount.at(bufferKey),
                                                     &subtreeAvailableChildCount.at(bufferKey),
                                                     subtreeRootLevel,
                                                     subtreeRootX,
                                                     subtreeRootY);
                }
            ...
            for (auto &[key, buffer] : subtreeBuffers) {
                ...
                bool constantNodeAvailability = (availableNodeCount == 0)
                                                || (availableNodeCount == subtreeNodeCount);
                bool constantChildAvailability = (availableChildCount == 0)
                                                 || (availableChildCount == childSubtreeCount);
                ...
                long unsigned int byteLength = static_cast<int>(!constantNodeAvailability)
                                                   * availabilityByteLength
                                               + static_cast<int>(!constantChildAvailability)
                                                     * childSubtreeAvailabilityByteLength;
                ...
                if (constantNodeAvailability) {
                    ...
                    subtreeJson["bufferViews"].push_back({{"buffer", 0},
                ...
                *(uint64_t *) &outBuffer[16] = bufferByteLengthAccum;          // BIN byte length with padding
                ...
                if (!constantNodeAvailability) {
                    memcpy(&outBuffer[outBufferByteOffset],
                           nodeAvailabilityBuffer,
                           nodeAvailabilityByteLengthWithPadding); // BIN node availability (already padded with 0s)
                    outBufferByteOffset += nodeAvailabilityByteLengthWithPadding;
                }
                if (!constantChildAvailability) {
                    memcpy(&outBuffer[outBufferByteOffset],
                           childSubtreeAvailabilityBuffer,
                           childSubtreeAvailabilityByteLengthWithPadding); // BIN child subtree availability (already padded with 0s)
                    outBufferByteOffset += childSubtreeAvailabilityByteLengthWithPadding;
                }
                std::filesystem::path path = geoCellAbsolutePath / "Elevation" / "subtrees"
                                             / (key + ".subtree");
